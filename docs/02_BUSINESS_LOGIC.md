...
| 创建退款请求 | 工作人员提议 Action、独立 manager/admin 批准且执行版本匹配后 | 模拟创建有副作用的退款处理请求 | 写操作不能仅靠 LLM 意图、Prompt 或 RBAC 直接放行 |
...
当前 Tool 使用本地 Mock 数据模拟 CRM、OMS 和工单系统。高风险 Action 参数加密保存，审计只保留 HMAC、字段名和脱敏摘要。这些能力用于说明业务上下文补全和调用治理方式，不代表系统已经连接真实企业服务，也不代表 AI 已经执行真实退款或订单修改。
...
| 高风险或写 Tool 超时 | 否 | 将 Action 标记为 `unknown`，保留审计和 Trace，等待对账 | 超时不等于外部未执行，盲目重试可能重复退款 |
...
退款、订单和审批都可能具有副作用。因此系统只对可识别的瞬时故障做有界 Retry，对高风险/非幂等写操作严格禁止自动重试。未来真实写 Tool 还必须具备业务幂等键、Outbox 和结果对账。
...
3. 执行时再次校验角色、Action 版本、Ticket 归属、密文和 HMAC，然后先持久化 `executing` 再调用 Mock OMS。
4. 成功进入 `succeeded`，确定性拒绝进入 `failed`，超时或结果不确定进入 `unknown` 并等待对账，绝不自动再退一次。

**为什么这样设计**：审批和执行是两个独立业务事件。持久化状态、职责分离和不确定结果建模，可以避免将“没收到成功响应”误判为“外部一定没执行”。
...
| 什么时候 Retry？ | 仅可识别的 LLM/RAG/低风险读 Tool 瞬时故障做有界 Retry；写 Tool 禁止 | 恢复瞬时故障同时避免重复副作用 |
| 什么时候人工介入？ | 安全、紧急、负面高优先级、低 QA、幻觉等 | 让人类保留最终业务裁决权 |
| 什么时候暂停/恢复？ | 需要审批时在 Approval Gate 暂停；决策持久化后从原执行点恢复 | 等待不占用请求，且不重复 LLM/RAG/Tool |
| 什么时候结束？ | 无需审批时直接结束；需审批时恢复并记录决策后结束；工单关闭才是业务闭环结束 | 防止把生成回复误当成问题已解决 |

## 17. 业务边界

- 当前工具、订单、客户和历史工单均为本地 Mock，不代表真实企业系统联动；高风险 Action 已建模，但还没有业务幂等键、Outbox 和自动对账 Worker。
- 当前没有 MCP、动态 Planner、自动 Reflection、分布式 Circuit Breaker / Queue / DLQ 或多租户知识隔离；LLM/RAG/Tool 有单进程有界恢复，审批等待已有持久化 Checkpoint 与数据库租约。
